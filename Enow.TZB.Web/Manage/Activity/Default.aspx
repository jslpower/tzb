<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/FinaWinBackPage.Master"
    AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="Enow.TZB.Web.Manage.Activity.Default" %>
        ...
        <div class="searchbox fixed">
            <span class="searchT">
                <p>
                    ...
                    &nbsp;&nbsp;时间：
                    <input type="text" onfocus="WdatePicker()" name="txtStartTime" value='<%=Enow.TZB.Utility.Utils.GetQueryStringValue("sttime")%>'
                        id="txtStartTime" size="10" class="inputtext" />-<input type="text" onfocus="WdatePicker({minDate:'#F{$dp.$D(\'txtStartTime\')}'})" class="inputtext" name="txtEndTime" value='<%=Enow.TZB.Utility.Utils.GetQueryStringValue("ettime")%>' id="txtEndTime" size="10" />
            ...
            <div id="tablehead" class="tablehead">
                <ul class="fixed">
                    ...
                    <asp:PlaceHolder ID="PlaceHolder1" runat="server" Visible="true">
                        <li class="line"></li>
                        <li><s class="delicon"></s><a href="#" hidefocus="true" class="Getbaoming">查看报名人数</a></li>
                        <li class="line"></li>
                    </asp:PlaceHolder>
                </ul>
                <div class="pages">
                    ...
                    <asp:Repeater ID="rptList" runat="server">
                        <ItemTemplate>
                            <tr>
                                ...
                                <td align="center">
                                    <%#string.IsNullOrEmpty(Eval("Starname").ToString())?"":"【"+Eval("Starname")+"】"%><%#Eval("Title")%>
                                </td>
                                <td align="center">
                                    <%#(Enow.TZB.Model.EnumType.ReleaseEnum)(Enow.TZB.Utility.Utils.GetInt(Eval("Release").ToString()))%>
                                </td>
                                <td align="center">
                                ...
                                <td align="center">
                                    <%#Enow.TZB.Utility.Utils.GetDateTime(Eval("StartDatetime").ToString()).ToString("yyyy-MM-dd")%>
                                </td>
                            </tr>
                        </ItemTemplate>
                    </asp:Repeater>
    ...
    <script type="text/javascript">
        var PageJsDataObj = {
            ...
            Search: function () {
                ...
                var sttime=$("#txtStartTime").val();
                var ettime=$("#txtEndTime").val();
                var fbmb=$("#<%=dropfbmb.ClientID %>").val();
                var types=$("#<%=droptypes.ClientID %>").val();
                window.location.href = "Default.aspx?"+$.param(PageJsDataObj.Query)+"&Title=" + Title + "&sttime=" + sttime+ "&ettime=" + ettime+"&fbmb="+fbmb+"&types="+types;
            },
            BindBtn: function () {
                //查看报名人数
                $(".Getbaoming").click(function() {
                   PageJsDataObj.Getbaoming();
                   return false;
                });
                //添加
