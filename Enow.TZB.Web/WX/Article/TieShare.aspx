<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TieShare.aspx.cs" Inherits="Enow.TZB.Web.WX.Article.TieShare" %>
        ...
        function pullUpAction() {
            setTimeout(function () {	// <-- Simulate network congestion, remove setTimeout from production!
                ...
                if (pageEnd == "0") {
                    ...
                    if (title == "")
                        title = "";
                    var para = { title: title, Page: index, types: 2, CityId: CityId };
                    $.ajax({
                        type: "Get",
...
<body>
    ...
    <uc1:UserHome ID="UserHome1" Userhometitle="培训堂" runat="server" />
    <div class="search_head juhui_search">
        <div class="search_form">
            <span class="city_name" onclick="location.href='/WX/city.aspx?action=Article&fhdz=TieShare&types=2'">
    ...
    <div class="TabTitle" style="margin-top: 105px;">
        <ul class="fixed">
            <li class="active"><a href="/WX/Article/TieShare.aspx">培训预约</a></li>
            <li><a href="/WX/Mall/Mall_Type.aspx?type=3">课程购买</a></li>
            <li><a href="/WX/Rudder/Default.aspx?types=2">堂主风采</a></li>
        </ul>
    </div>
    <div class="warp s_warp" id="wrapper" style="margin-top: 150px;">
                    ...
                    <ItemTemplate>
                        <li>
                            <div class="juhui-title">
                                <%# string.IsNullOrEmpty(Eval("Starname").ToString())?"":"<em>["+Eval("Starname")+"]</em>"%><%#Eval("Title")%></div>
                            <div class="juhui-box">
                                <div class="btn">
                                    <a href="TZGatheringDetail.aspx?ActId=<%#Eval("Id")%>&types=<%#Eval("Activitytypes")%>"
                                        class="basic_btn">查看</a> <a href="javascript:void(0);" data-actid="<%#Eval("Id")%>"
                                            class="basic_ybtn butbaom">报名</a>
                                </div>
                                <p class="font_gray">
                                    时间：<%#Enow.TZB.Utility.Utils.GetDateTime((Eval("StartDatetime")).ToString()).ToString("yyyy-MM-dd")%></p>
                                <%#Enow.TZB.Utility.Utils.GetInt(Eval("Activitytypes").ToString()) <= 2 ? "<p>球场:" + Eval("SiteName") + "</p>" : ""%>
                                <p>
                                    地址：<%#Eval("Countryname")%><%#Eval("Provincename")%><%#Eval("Cityname")%><%#Enow.TZB.Utility.Utils.GetInt(Eval("Activitytypes").ToString()) <= 2 ? "" : Eval("Areaname")%><%#Eval("Address")%></p>
                                <p class="font_gray">
                                    费用：<%#Eval("CostNum")%></p>
                            </div>
                        </li>
    ...
    <script type="text/javascript">
        var PageJsDataObj = {
            ...
            Add: function (obj) {
                ...
                if (atid) {
                    var dataurl = "/WX/Member/TZGatheringDetail.aspx?ation=inter&ActId=" + atid;
                    this.GoAjax(dataurl);
                }
                else {
                    alert("请选择要报名的活动!");
