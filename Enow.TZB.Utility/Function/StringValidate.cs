...
namespace Enow.TZB.Utility
{
	...
	{
        private static string _UploadFileExt = ".txt,.gif,.bmp,.png,.jpg,.jpeg,.doc,.docx,.pdf,.xls,.xlsx,.ppt,.pptx,.rar,.zip";
        private static string _UploadImgExt = ".gif,.bmp,.png,.jpg,.jpeg";
        ...
        private static Regex RegIDcard = new Regex(@"(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)");
        /// <summary>
        /// 小灵通号码
        /// </summary>
...
//			RandNum = rnd.Next(1,10000);//生成一个10000以内的随机数
			PhotoName = Guid.NewGuid().ToString().Replace("-","") + FileExt;
			return PhotoName;
		}
		/// <summary>
		/// 返回当日的日期名称
        ...
        /// <param name="CheckExtArr">所要检查的后缀</param>
        public static bool CheckFileExt(string FileExt, params string[] CheckExtArr)
        {
            foreach (string fFileExt in CheckExtArr)
            {
                if (FileExt.ToLower().Trim() != fFileExt.ToLower().Trim())
                {
                    return false;
                }
            }
        ...
        {
            string tmpFileExt = _UploadImgExt;
            string[] strFileExt = tmpFileExt.Split(',');
            foreach (string fFileExt in strFileExt)
            {
                if (FileExt.ToLower().Trim() == fFileExt.Trim())
