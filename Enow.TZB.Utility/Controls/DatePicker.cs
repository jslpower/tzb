...
namespace Enow.TZB.Utility.DatePicker
{
	/// <summary>
	/// 新版日期选择控件
	/// </summary>
	...
	public class DatePicker : System.Web.UI.WebControls.WebControl ,INamingContainer
	{
		...
		protected override void OnPreRender(EventArgs e)
		{
			...
			*/
			...
			{
				this._dateTextBox.Attributes["onfocus"] = "WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})";
			}
			else
			{
			...
			if(this.DisplayTime)//显示时间
			{
				this._calendarImage.Attributes["onclick"] = "WdatePicker({el:$dp.$('" + this._dateTextBox.ClientID + "',dateFmt:'yyyy-MM-dd HH:mm:ss')})";
			}
			else
			{
