...
var SupplierType = {
    ...
    _post: function (t, options, pv, fn) {
        ...
        switch (t) {
            case "g": _$obj = this._getObj(options.gid); _data = { "get": "g" }; _sv = options.gv; break;
            case "s": _$obj = this._getObj(options.sid); _data = { "get": "p", "gid": pv }; _sv = options.sv; break;
            case "p": _$obj = this._getObj(options.cid); _data = { "get": "s", "pid": pv }; _sv = options.cv; break;
            default: break;
        }
        ...
        $.ajax({
            type: "get", cache: false, dataType: "json",
            url: "/ashx/GetSupplierType.ashx?" + $.param(_data),
            success: function (response) {
                if (response && response.list != null) {
                    _self._setSelectOptions(_$obj, response.list, _sv);
                    if (fn) { fn(); }
...
var stToobar = {
    ...
    init: function (options) {
        var _options = { gid: options.gID, sid: options.pID, cid: options.cID, gv: options.gSelect, sv: options.pSelect, cv: options.cSelect }
...
var FSupplierType = {
    ...
    //发起请求 t:请求类型 g一级类别 s二级类别 p 供应商 f 发行方 pv:上一级 fn:回调函数
    _post: function (t, options, pv, fn) {
        var _self = this; var _$obj = null; var _data = {}; var _sv = "";
        ...
        switch (t) {
            ...
            case "s": _$obj = this._getObj(options.sid); _data = { "get": "p", "gid": pv }; _sv = options.sv; break;
            case "p": _$obj = this._getObj(options.cid); _data = { "get": "s", "pid": pv }; _sv = options.cv; break;
            case "f": _$obj = this._getObj(options.fid); _data = { "get": "f", "pid": pv }; _sv = options.fv; break;
            default: break;
        }
    ...
    //设置发行方
    _setF: function (options) {
        var _sv = "1";
        var _$s = this._getObj(options.sid);
        if (_$s) _sv = _$s.val();
        var _self = this;
        this._post("f", options, _sv, null);
    },
    //设置二级类别
    _setS: function (options) {
        var _gv = "1";
        ...
        if (_$g) _gv = _$g.val();
        var _self = this;
        this._post("s", options, _gv, function () { _self._setC(options); _self._setF(options); });
    },
    //设置一级类别
    _setG: function (options) {
        var _self = this;
    ...
    //options={gid:"#一级类别下拉菜单",sid:"#二级类别下拉菜单",cid:"#供应商下拉菜单",fid:"#发行方下拉菜单",gv:"一级类别选中的值",sv:"二级类别选中的值",cv:"供应商选中的值",fv:"发行方选中的值"}
    init: function (options) {
        var _options = { gid: "", sid: "", cid: "", fid: "" };
        options = $.extend(_options, options);
        var _$g = this._getObj(options.gid);
        var _$s = this._getObj(options.sid);
        var _$c = this._getObj(options.cid);
        var _$f = this._getObj(options.fid);
        var _self = this;
        if (_$g) { _$g.bind("change", function () { _self._setS(options); }); }
        if (_$s) {
            _$s.bind("change", function () {
                _self._setF(options);
                _self._setC(options);
            });
        }
...
var stFToobar = {
    ...
    init: function (options) {
        var _options = { gid: options.gID, sid: options.pID, cid: options.cID, fid: options.fID, gv: options.gSelect, sv: options.pSelect, cv: options.cSelect, fv: options.fSelect }
        FSupplierType.init(_options);
    }
};
//产品组合选择
var ProductSelect = {
    ...
    _post: function (t, options, pv, fn) {
        ...
        switch (t) {
            case "g": _$obj = this._getObj(options.gid); _data = { "get": "g" }; _sv = options.gv; break;
            case "s": _$obj = this._getObj(options.sid); _data = { "get": "s", "gid": pv }; _sv = options.sv; break;
        ...
        $.ajax({
            type: "get", cache: false, dataType: "json",
            url: "/ashx/GetProductSelect.ashx?" + $.param(_data),
            success: function (response) {
                if (response && response.list != null) {
                    _self._setSelectOptions(_$obj, response.list, _sv);
                    if (fn) { fn(); }
    ...
    //options={gid:"#一级类别下拉菜单",sid:"#二级类别下拉菜单",pid:"#产品系列下拉菜单,zid:"#产品组合下拉菜单,mid:"#产品组合金额下拉菜单",gv:"一级类别选中的值",sv:"二级类别选中的值",pv:"产品系列选中的值",zv:"产品组合选中的值",mv:"产品组合金额选中的值"}
    init: function (options) {
        var _options = { gid: "", sid: "", pid: "", zid: "", mid: "", gv: "", sv: "", pv: "", zv: "", mv: "" };
        options = $.extend(_options, options);
        var _$g = this._getObj(options.gid);
        var _$s = this._getObj(options.sid);
        var _$p = this._getObj(options.pid);
...
var psToobar = {
    ...
    init: function (options) {
        var _options = { gid: options.gID, sid: options.sID, pid: options.pID, zid: options.zID, mid: options.mID, gv: options.gSelect, sv: options.sSelect, pv: options.pSelect, zv: options.zSelect, mv: options.mSelect }
